...
VAR="EventBDT"          # ex: MET             Plots[0]="STW_6Bins"; Plots[0]="VtxBDT_Ave";  Plots[0]="EventBDT";
...
# === Valeurs de masses ===
declare -a SMU_VALUES=(600)
declare -a NEU_VALUES=(185 200 250 300 400 500 550 580)
...
for NEU in "${NEU_VALUES[@]}"; do
    ...
    if [[ $SMU -eq 200 && $NEU -eq 180 ]]; then
        :
    elif [[ $SMU -eq 600 && ( $NEU -eq 185 || $NEU -eq 200 || $NEU -eq 250 || $NEU -eq 300 || $NEU -eq 400 || $NEU -eq 480 ) ]]; then
        :
    else
    ...
    source ./CombineDataCards_600.sh $YEAR $VAR $SMU $NEU $CTAU_VAL
