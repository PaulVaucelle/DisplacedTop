...
double RLLR13TEV[nMC] = {0.004482*2*1000,0.001888*2*1000,0.0009079*2*1000,0.0004742*2*1000,0.0002646*2*1000,0.0001551*2*1000,9.443e-05*2*1000};
...
double K_RLLR13TEV[nMC] = {0.004482*2*1.4*1000,0.001888*2*1.4*1000,0.0009079*2*1.4*1000,0.0004742*2*1.48*1000,0.0002646*2*1.4*1000,0.0001551*2*1.4*1000,9.443e-05*2*1.4*1000};
   ...
   auto gr = new TGraphErrors(nMC,Msmu,RLLR13TEV,ex,ERROR_RLLR13TEV);
...
double RLLR13TEV1J[nMC] = {0.007178*2*1000,0.003125*2*1000,0.001533*2*1000,0.000821*2*1000,0.0004676*2*1000,0.0002767*2*1000,0.0001709*2*1000};
double ERROR_RLLR13TEV1J[nMC] = {1.535e-02,1.05e-02,3.632e-03,2.406e-03,1.022e-03,6.861e-04,4.128e-04};
double K_RLLR13TEV1J[nMC] = {0.007178*2*1.4*1000,0.003125*2*1.4*1000,0.001533*2*1.4*1000,0.000821*2*1.4*1000,0.0004676*2*1.4*1000,0.0002767*2*1.4*1000,0.0001709*2*1.4*1000};
    auto gr2 = new TGraphErrors(nMC,Msmu,RLLR13TEV1J,ex,ERROR_RLLR13TEV1J);
...
double RLLR14TEV1J[nMC] = {0.008224*2*1000,0.003638*2*1000,0.001799*2*1000,0.0009728*2*1000,0.0005624*2*1000,0.000336*2*1000,0.0002104*2*1000};
double ERROR_RLLR14TEV1J[nMC] = {1.671e-02,8.147e-03,5.016e-03,2.734e-03,1.277e-03,9.81e-04,5.457e-04};
double K_RLLR14TEV1J[nMC] = {0.008224*2*1.4*1000,0.003638*2*1.4*1000,0.001799*2*1.4*1000,0.0009728*2*1.4*1000,0.0005624*2*1.4*1000,0.000336*2*1.4*1000,0.0002104*2*1.4*1000};
    auto gr4 = new TGraphErrors(nMC,Msmu,RLLR14TEV1J,ex,ERROR_RLLR14TEV1J);
...
gr4->Draw("LPSAME");
  ...
  leg1->SetTextSize(0.04);
    ...
    leg0->AddEntry(gr3,"RL+LR LO 14TeV ");
    leg0->AddEntry(gr,"RL+LR LO 13TeV ");
    leg0->Draw();
    leg1->Draw();

 c1->Update();

 c1->SaveAs("CrossSectionRLLR.pdf");
 c1->SaveAs("CrossSectionRLLR.png");
